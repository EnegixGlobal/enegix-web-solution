...
interface TeamMember {
  ...
  experience: number;
  skills: string[];
  department: 'leadership' | 'development' | 'sales' | 'marketing' | 'design' | 'other';
  joinDate: string;
...
const TeamMemberCard = ({ member }: { member: TeamMember | typeof foundersData[0] }) => {
  ...
  return (
    ...
    >
      <div className="h-[380px] relative overflow-hidden rounded-t-2xl">
        <Image
        ...
        <div className="absolute bottom-4 left-4 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          {member.socialLinks.linkedin && (
            <a
              ...
              className="p-2 bg-white/90 hover:bg-white rounded-full transition-all duration-300 hover:scale-110"
            >
          ...
          {member.socialLinks.facebook && (
            <a
              ...
              className="p-2 bg-white/90 hover:bg-white rounded-full transition-all duration-300 hover:scale-110"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" className="text-blue-600">
          ...
          {member.socialLinks.instagram && (
            <a
              ...
              className="p-2 bg-white/90 hover:bg-white rounded-full transition-all duration-300 hover:scale-110"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" className="text-pink-600">
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-2">
          ...
          {isTeamMember && (member as TeamMember).experience > 0 && (
            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
              {(member as TeamMember).experience}+ years
            </span>
          )}
        </div>
        ...
        {isTeamMember && (member as TeamMember).department && (
          <p className="text-sm text-gray-500 mb-3 capitalize">
            📂 {(member as TeamMember).department}
          </p>
        )}
        ...
        {/* Skills */}
        {isTeamMember && (member as TeamMember).skills && (member as TeamMember).skills.length > 0 && (
          <div className="mb-4">
            <p className="text-xs text-gray-500 mb-2">Skills:</p>
            <div className="flex flex-wrap gap-1">
              {(member as TeamMember).skills.slice(0, 3).map((skill, index) => (
                <span
                  key={index}
              ...
              {(member as TeamMember).skills.length > 3 && (
                <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">
                  +{(member as TeamMember).skills.length - 3} more
                </span>
              )}
            </div>
          </div>
        ...
        {/* Join Date */}
        {isTeamMember && (member as TeamMember).joinDate && (
          <p className="text-xs text-gray-400">
            Joined: {new Date((member as TeamMember).joinDate).toLocaleDateString('en-US', { 
              month: 'short', 
              year: 'numeric' 
            })}
          </p>
...
export default function TeamPage() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const response = await fetch('/api/team?status=active&featured=false&limit=50&sort=order,name');
  ...
  const filteredEmployees = employees.filter((member) => {
    ...
    if (activeFilter === "development")
      return member.role.toLowerCase().includes("developer") || 
    ...
    if (activeFilter === "sales")
      return member.role.toLowerCase().includes("sales") || 
  ...
  const shouldShowFounders = activeFilter === "all" || activeFilter === "leadership";
          ...
          <Container>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
                ...
                <div className="text-center p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                  <div className="text-3xl font-bold text-blue-600 mb-2">
                    {employees.filter(m => m.department === 'development').length}
                  </div>
                  <p className="text-gray-600 text-sm">Developers</p>
                </div>
                <div className="text-center p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                  <div className="text-3xl font-bold text-green-600 mb-2">
                    {employees.filter(m => m.department === 'sales' || m.department === 'marketing').length}
                  </div>
                  <p className="text-gray-600 text-sm">Sales & Marketing</p>
                </div>
                <div className="text-center p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                  <div className="text-3xl font-bold text-purple-600 mb-2">
                    {Math.round(employees.reduce((acc, m) => acc + m.experience, 0) / employees.length) || 0}
                  </div>
                  <p className="text-gray-600 text-sm">Avg. Experience (Years)</p>
                </div>
              </div>
            )}
            ...
            {loading ? (
                ...
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="animate-pulse bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    <div className="p-6">
                      ...
                      <div className="bg-gray-200 h-3 rounded w-1/3 mb-3"></div>
                      ...
                      <div className="flex gap-1 mb-4">
                        ...
                        <div className="bg-gray-200 h-6 rounded-full w-14"></div>
                      </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-red-600 text-lg mb-2">Error loading team members</p>
            ...
            {!loading && !error && regularMembers.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 text-lg">
                  {activeFilter === "all" ? "No employees found." : "No team members match the selected filter."}
                </p>
              </div>
            )}
          </Container>
